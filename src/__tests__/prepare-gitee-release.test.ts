...
const toAbsolute = (name: string) => `https://gitee.com/ChiangyangNPU/tmd/releases/download/v0.1.0/${name}`
...
describe('absolutizeYmlUrls', () => {
  it('files 条目的 url 与顶层 path 都改为绝对 URL', () => {
    ...
    expect(out).toContain('- url: https://gitee.com/ChiangyangNPU/tmd/releases/download/v0.1.0/TMD-0.1.0-x64-Setup.exe\n')
    expect(out).toContain('path: https://gitee.com/ChiangyangNPU/tmd/releases/download/v0.1.0/TMD-0.1.0-x64-Setup.exe')
    // blockmap 同样被处理
    expect(out).toContain('- url: https://gitee.com/ChiangyangNPU/tmd/releases/download/v0.1.0/TMD-0.1.0-x64-Setup.exe.blockmap')
  })
  ...
  it('releaseNotes 正文里形如 url/path 的普通文本不被误替换（无行首锚点不匹配）', () => {
    const withText = WIN_YML.replace('  首个正式发布版本。', '  修复了 url: 解析问题与 path: 拼接问题')
