...
后续补充的历史版本 6 断言（见 §25.5）插在「另存为」与「未保存关闭」之间——「另存为」会把激活标签切到新文件，故该段先把标签切回原文档（同路径已打开时 `openFromData` 走 `activateTab` 去重）再验证快照，脚本现共 17 断言。
...
- 桌面 E2E 6 断言：写盘留档 → 菜单开面板并列项 → 预览为旧版正文 → 恢复置脏且面板自动关闭 → 磁盘未被恢复动作改写
...
| 跨文件搜索的定位范围 | 所见即所得与源码模式均可定位到匹配处并选中（源码模式经 `jumpToSourceMatch`，见 §22） | — |
...
| 主题预设 / 自定义 CSS | 已支持：内置四项预设 + 文件式主题目录（`~/.tmd/themes/*.css`，设置面板选择/手动刷新）+ 自定义 CSS 文本域（见 §12） | — |
| 导出 Word | Word 生成为**原生可编辑 OOXML**（标题/段落/列表/表格/行内格式/超链接均为原生元素，可用 Word 继续编辑）；独占公式与 Mermaid 图表为 2x 位图快照（不可编辑），行内公式保留可编辑文本；远程图片仅支持 png/jpg/gif/bmp（webp/avif/svg 回退 alt 文本）；高于视口的超大图表不栅格化（保留原 SVG，转换时丢弃）（见 §7） | 视反馈评估公式的可编辑化（OMML） |
| 导出长图 | 单张 PNG，物理清晰度 ≥ 2x，按纹理上限自动分段拼接；整图物理高上限 120000px，超限拒绝并提示（见 §7） | — |
