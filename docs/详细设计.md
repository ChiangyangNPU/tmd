...
- 切换语言时同步五处：静态文案、`<html lang>`（同一字体下的简/繁字形选择、断行与屏幕阅读器发音都据此判定，index.html 的静态 `lang="zh-CN"` 会被纠正）、标签栏、标题与字数、主进程菜单文案
- 导出 HTML 的 `<html lang>` 同样取当前语言，保证导出页用对字形
- 菜单重建时保留自动保存勾选状态（主进程持有）
- 已知小缺口：主进程读目录时按 `localeCompare(..., 'zh-CN')` 排序文件名（拼音序），繁中用户下未改用注音/笔画序——影响仅限文件树内中文文件名的排序
