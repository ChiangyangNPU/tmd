...
- 匹配：把文档按「连续文本段」切开——文本块（段落 / 标题 / 代码块）边界与行内原子节点（图片 / 脚注引用 / 硬换行）都会断开，段内拼接线性文本并记录每个文本节点到文档位置的映射，匹配后再反映射回 ProseMirror 位置。因此可跨格式标记匹配（`**bo**ld` 能搜到 bold），但不跨段落、不跨越原子节点（与 Typora 同一取舍：替换不会合并段落）
- 正则模式：查找栏「.*」按钮切换，表达式以 `gi` 编译（大小写不敏感固定开启，与普通查找语义一致）；匹配结果带命中原文与捕获组，替换串支持 `$&` / `$1-$9` / `$$`；表达式非法时在计数位红字提示且不改动文档
- 匹配上限：单次最多 2000 条，避免正则回溯或超长文档下逐项高亮拖垮界面；可产生空匹配的表达式（如 `a*`）前移一位继续，不会死循环
- 小写化兼容：`toLowerCase` 改变长度的字符（如 `İ` → `i̇`）会让小写串下标无法映射回原文，此时退回大小写敏感匹配，保证区间落在原文准确位置
...
- 替换单个：先清空匹配列表（避免该帧装饰器沿用已被替换掉的旧区间），再 `tr.insertText(展开后的替换词, from, to)`；随后重算并把序号停在原位（即下一个匹配），便于连续替换
...
- **Word（src/export-word.ts）**：`dom-docx` 浏览器入口 + `styleSource:'computed'`（直接从 live DOM 读计算样式，导出版式在样式表与 CSS 变量里，无需自写"样式内联层"）。`pre.mermaid` 图表用真实合成器区域截帧栅格化再替换元素（其 SVG 含 foreignObject，交给转换器的 canvas 栅格化会因画布污染抛错；自绘 foreignObject 又取不到网页字体）；截帧前需 `scrollIntoView` 且元素完整落在视口内，失败即保留原 DOM 降级；超出 120 块的图表降级为源码文本。转换器选项：`pageSize:'a4'`（中文文档惯例）、`lang` 取当前界面语言、`rasterizeInPlace:false`、`onWarning` 收集降级告警。转换器本体藏在单一函数之后，替换引擎只需改本文件
- **Word 公式（src/export-omml.ts，OMML 可编辑化）**：独占公式不再栅格化为位图，而是导出后改写 `word/document.xml` 注入 Word 原生公式对象——dom-docx 不支持公式、也没有 raw OOXML 扩展点，故在 HTML 阶段先把 `.katex-display` 换成占位段落（文本形如 `@@TMDMATH0@@`，LaTeX 源码取自 KaTeX MathML 的 `annotation`），转换完成后解压 docx、按占位定位并把**整段**替换为 `<m:oMathPara>`（居中），必要时在根元素补声明 `xmlns:m`，再重新打包。链路为「LaTeX →（KaTeX，`output:'mathml'`）MathML →（mathml2omml）OMML」；任一环节失败时该段降级为居中显示的 `$$…$$` 源码文本，绝不产出无法阅读的内容。定位段落不能直接 `lastIndexOf('<w:p')`——`<w:pPr` / `<w:pStyle` 同前缀，需回溯校验标签名后紧跟 `>` 或空白
...
防抖参数的标定教训：**防抖时长必须大于 bench 的按键间隔**（bench 每键含两次 CDP 往返，约 600-800ms）——防抖短于按键间隔时，序列化恰好落在测量窗口内，bench 中位数看不出收益；防抖 1200ms（大于按键间隔）时又暴露另一陷阱：max-stale 兜底在每次按键后都以 0 延迟重排，连续输入退化为逐键全量序列化，中位数反而恶化到 406ms。最终取 800ms。**剩余基线**：消去钩子成本后，纯文本大文档的输入同步仍有约 73ms，为 ProseMirror 事务处理与原生布局的地板（与路线图「虚拟化不做」同源），门禁按 Windows 平台系数放行。该 73ms 是标定当时的机器状态，随负载浮动，不可当常量用（复核过程见 §26.6）。

### 26.6 输入门限漂移复核（判代码退化还是机器负载）

门禁（`60 × 1.5 = 90ms`）在当前 HEAD 上连续失败，实测 86 / 91 / 93 / 97 / 106ms（5 次 bench，中位 93），而标定文档记的 73-77ms 已不可复现。放宽门限之前必须先分清是代码退化还是环境漂移，方法是**同机同时段构建两个版本交替跑**：

```bash
git worktree add --detach ../tmd-ab <标定提交>   # 351d454
# node_modules 用目录联接复用主仓（Windows 下 mklink /J，勿用 cp -r）
npm run build && node scripts/desktop-bench.mjs --json
```

| 构建 | bench 场景 A 输入同步中位 | 文档渲染完成后单键同步中位 |
|---|---|---|
| 351d454（标定提交） | 87.1 / 89.1 ms | 26.2 ms |
| 当前 HEAD | 86.4–106.3 ms（中位 93） | 26.8 ms |

两点结论：**标定提交在今日同机也测到 87-89ms**，绝对耗时随机器负载漂移；而"渲染完成后的单键成本"两构建只差 2%，说明每键同步处理路径没有退化。差异来自 bench 场景 A 的测量窗口本身——它在"内容就绪"（1MB 文档就绪 368K 字符）后立刻按键，样本混着尚未完成的剩余 DOM 构建，故绝对值必然随负载起伏（同一构建极差 ±11%）。

**处置**：门限改为按实测分布取宽松上界（`80 × 平台系数`，Windows 120ms ≈ 中位的 1.3 倍），并在 `desktop-bench.mjs` 的 `LIMITS` 注释里连同本次对照数据一起留档；这样既不误报，退化到逐键全量序列化（+90ms 量级）时仍能拦住。CPU 剖析（`scripts/profile-input.mjs`）同期无新增热点：采样 82.6% idle，最高 JS 自耗时项是 prosemirror 序列化 chunk 的 4.4%。
...
| 项 | 现状 | 计划 |
|---|---|---|
| 大文档输入延迟 | 优化后剩余地板为 ProseMirror 事务处理 + 原生布局（本机实测：文档渲染完成后单键同步 26ms；bench 场景在首屏渲染未完时按键，86–106ms 且随机器负载起伏，见 §26.6；虚拟化已决策不做，见 §26.5）；序列化在暂停 800ms 后单次执行，1MB 文档单次约 80-400ms，为低优调度的刻意取舍 | 视反馈评估是否值得进一步压缩（需 PM 层改造） |
...
| 查找替换跨节点与正则 | 段内跨格式标记匹配（不跨段落与行内原子节点）；「.*」正则模式含捕获组替换（$1 / $&）、非法表达式红字提示、2000 条匹配上限 | §5 |
| 导出 Word 公式 | 独占公式由 2x 位图改为 OMML（Word 原生可编辑公式），转换失败该段降级为 LaTeX 源码文本；Mermaid 图表仍为位图 | §7 |
...
> - 2026-09-23　新增 §26.6 输入门限漂移复核（worktree 同机对照标定提交，判明为机器负载而非代码退化；门禁改为按实测分布取宽松上界），§26.5 与 §35 的地板耗时改为标注测量窗口
