...
- **HTML**：markdown-it 渲染正文（`html:false` 防 XSS；mermaid 围栏转为 `<pre class="mermaid">` 由 CDN 脚本渲染；KaTeX 走 CDN 自动渲染；另接入 footnote / mark / sub / sup 四个 markdown-it 插件支持脚注与扩展行内语法，脚注引用/定义与 `<mark>/<sup>/<sub>` 均有导出样式）；渲染前 `stripFrontMatter` 剥离文档开头 front matter（.md 源文件始终保留）；样式内联；Electron 下走 `export-as` 保存对话框，浏览器下降级为下载
...
**快捷键配置源（src/shortcuts.ts）**：全部 29 项快捷键（文件 6 / 导出 2 / 格式 17 / 编辑器 4）集中定义在 `SHORTCUT_DEFS`，每项含 action、显示名 i18n 路径、默认 accelerator、分组。配置以渲染层 localStorage 为权威，主进程经 `tmd:sync-shortcuts` 单向接收后重建菜单 accelerator；编辑器侧 `handleKeyDown` 每次按键实时读同一份配置。因此改键后三处（主进程菜单 / 所见即所得编辑器 / 全局 keydown）同时生效，无需重启或重建编辑器。
...
**跳转与定位**：主进程返回的是**原始 markdown 的行号**，与 ProseMirror 位置不可直接换算（frontmatter 在挂载时被剥离，`#`/`*` 等语法字符不占 PM 位置）。因此跳转链路为：

1. `openPath` / `activateTab` 打开目标文件（两者内部都 `await` 了编辑器重建，返回即编辑器已就绪）
2. 用现成的 `findMatches(view.state.doc, query)` 在同文档内重新匹配，取第 `occurrence` 个（匹配数不足时退回最后一个）
3. 设置选区并调用 `scrollEditorPosIntoView()` 滚动（PM 自带的 `scrollIntoView` 对 `.page-scroll` 无效）

**已知限制**：源码模式（CodeMirror）下不做定位——`editor-core` 刻意把 CodeMirror 实例的类型收窄为只暴露 `destroy` 与 `state.doc.toString`，没有跳转到行的接口；此时仅打开文件。
...
与 `link-nav.ts` 的 `normalizePath` 的分工：后者服务文档内链接（输入为 URL 风格路径，输出保证以 `/` 开头），前者服务本地文件系统路径。

## 24. 已知限制与后续计划

| 项 | 现状 | 计划 |
|---|---|---|
| 粘贴图片 | 已支持三策略：inline data URL（默认）/ assets 文件夹相对路径 / 图床上传（PicGo-Core，7 种图床），设置面板切换，失败自动降级 inline（见 §8） | — |
| 跨文件搜索的定位范围 | 所见即所得模式可定位到匹配处并选中；源码模式仅打开文件、不落位（editor-core 未暴露 CodeMirror 跳转接口，见 §22） | 如需支持，为 editor-core 增加 jumpToLine 访问器 |
| 快捷键自定义 | 已完成：29 项快捷键集中于 `src/shortcuts.ts`，设置面板可改键（捕获 / 合法性校验 / 冲突检测 / 恢复默认），主进程菜单与编辑器读同一份配置（见 §14） | — |
...
| 脚注 / front-matter | 已完成：脚注 schema 用 gfm preset 内置，输入规则（引用原子化 / 定义转换 / Enter 退出）自研于 mark-ext.ts（见 §4.4），导出接 footnote 插件；front matter 属性表双态、字节级往返、导出剥离；上下标序列化统一 Pandoc 风格 | — |
