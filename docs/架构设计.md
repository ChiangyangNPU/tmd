...
flowchart LR
    ...
    subgraph 编辑器内核
        ...
        MARKEXT[mark-ext.ts<br/>高亮 / 上下标语法扩展]
...
| `mark-ext.ts` | 语法扩展（remark 插件 + schema + 输入规则）：`==高亮==`、上/下标——`^x^`/`~x~` 与 `^{x}`/`_{x}` 双写法均识别，序列化统一为 Pandoc 单符号风格；行内代码/公式未闭合跨度内不激活（防误触） | `markPlugins`、`inUnclosedSpan` |
