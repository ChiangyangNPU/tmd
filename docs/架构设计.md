...
| `search.ts` | 跨文件全文搜索面板：收集已挂载文件夹为根目录、300ms 输入防抖、序号守卫丢弃过期结果、结果按文件分组渲染（关键词高亮）、回车/点击跳转；打开文件后用 `findMatches` 在同文档内重新匹配定位（源码行号与 PM 位置不可换算） | `openSearch`、`wireSearch`、`splitHighlights`、`groupMatches` |
...
| 全文搜索 | 主进程递归扫描已挂载文件夹（跳过依赖目录、限制单文件 2MB / 扫描 5000 文件 / 命中 500 行、8 路受限并发），渲染层 300ms 防抖 + 序号守卫丢弃过期结果；跳转按「同文档重新匹配」定位 | `electron/search.cjs` / `search.ts` |
