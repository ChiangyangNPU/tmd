...
│   ├── desktop-export-check.mjs # 导出 Word / 长图 / LaTeX 桌面 E2E（35 断言）
...
│   ├── export.ts                # 导出动作层（HTML / PDF / Word / 长图 / LaTeX）
│   ├── export-doc.ts            # 载体无关的导出文档核心（解析管线工厂 + 渲染管线 + 样式 + 图片引用分类）
│   ├── export-latex.ts          # LaTeX 适配器（token 流 → .tex，白名单渲染 + 捕获栈 + 文档组装）
...
| 构建脚本形态 | `scripts/*.mjs` 一律**不带 shebang**，只以 `node scripts/x.mjs` 调用（`package.json` 与 CI 全部如此）。shebang 只在裸文件交给内核执行时有意义，而 vite 的 SSR transform 会把源码包一层，`#!` 不再位于首行即成非法 token，vitest import 时报无堆栈的 `SyntaxError: Invalid or unexpected token`；纯函数段能被单测 import 是刻意的（发布链路的 CHANGELOG CRLF 切分、latest-mac.yml 的 sha512/size 重算靠它兜底） | `scripts/*.mjs` |
...
| 导出（五载体同源） | 解析管线工厂只有一份（`export-doc.ts` createExportMarkdownIt：插件集 / 公式保护 / 图片还原），各渲染目标适配器消费：HTML 套 CDN 外壳、PDF 走系统打印现成 DOM、Word / 长图由隐藏窗口消费同一份文档并在页面内完成转换/截图、LaTeX 由独立渲染器把 token 流翻译为 .tex（math_inline token 公式零失真透传）；主题变量快照在导出瞬间采集，深色与文件主题自动跟随 | `export.ts` / `export-doc.ts` / `export-latex.ts` / `export-renderer.ts` / `electron/exporter.cjs` |
...
> **版本历史**
>
> - 2026-09-23　§7 增「构建脚本形态」横切项：`scripts/*.mjs` 去 shebang（vite SSR transform 下 shebang 会让 vitest import 直接 SyntaxError），三份发布脚本的纯函数单测恢复可加载
> - 2026-09-19　样式按组件拆分（`src/styles/01-15`，编号即级联顺序，style.css 只留 @import 汇总）；index.html 浮层面板拆为 `src/templates/*`（?raw 内联 + boot 注入 `<template data-partial>` 占位）；主进程菜单模板抽至 `electron/menu.cjs`（纯数据工厂，可独立单测）；导出架构工厂化并新增 LaTeX 适配器（`export-latex.ts`，五载体同源）
